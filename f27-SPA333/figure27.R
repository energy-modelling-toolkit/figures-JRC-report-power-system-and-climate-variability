...
toplot <- read_csv("data_figure27.csv")
cmap <- c(
...
g <- ggplot(toplot, aes(x = h, y = OutputPower)) +
  geom_bar(
    data = toplot %>% filter(!(fuel_class %in% c("Demand", "Loss of Load"))),
    stat = "identity",
    aes(fill = fuel_class),
    width = 3600
  ) +
...
scale_x_datetime(
  ...
  ggrepel::geom_text_repel(
    data = toplot %>%
      filter(id == 2014, hour(h) == 20, fuel_class == "Demand"),
    aes(x = h, y = OutputPower), label = "Shed load: 1.2 GW",
    direction = "x",
    box.padding = 3,
    segment.colour = "black"
  )

ggsave("figure27.png", width = 6, height = 8)
